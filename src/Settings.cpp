...
void Settings::loadConfig(const char* config_file) {
    ...
    if(!in || !in.is_open()) {
        std::cerr << "Warning: couldn't load settings file: " << config_file << std::endl;
        return;
    }
...
string Settings::loadCommandLine(int argc, char** argv) {
  for(int i = 1; i < argc; ++i)
  {
    ...
    if(key[0] == '-')
    {
      ...
      if(++i >= argc)
      {
        std::cerr << "Missing argument for '" << key << "'" << endl;
        return "";
      }
      string value = argv[i];
    ...
    if((idx = getInternalPos(key)) != -1) {
        ...
        if((idx = getExternalPos(key)) != -1) {
            ...
            if (strict) {
                ...
                std::cerr << "Make sure all the settings files are loaded." << endl;
                exit(-1);
            } else {
                return -1;
            }
