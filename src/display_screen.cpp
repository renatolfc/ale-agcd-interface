...
DisplayScreen::DisplayScreen(AtariState* atariState,
        ...
        delay_msec(0)
{
    screen_height = atariState->height();
    screen_width = atariState->width();
    ...
    screen = (void *)SDL_SetVideoMode(window_width, window_height, 8, SDL_HWPALETTE);
    if (screen == NULL) {
...
void DisplayScreen::display_screen() {
    SDL_Surface *sdl_screen = reinterpret_cast<SDL_Surface *>(screen);
    ...
    uInt8* pi_curr_frame_buffer = atariState->getScreen().getArray();
    int y, x, r, g, b;
    SDL_Rect rect;
...
void DisplayScreen::handleSDLEvent(const void *event) {
